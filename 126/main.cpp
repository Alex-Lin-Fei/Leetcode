...
void backtracking(vector<string>& idStr, vector<vector<string>>& ans, vector<string>& temp,
                  vector<int>& high, vector<vector<int>> & next, int idx, int beginIdx) {
    if(idx == beginIdx) {
    ...
    for(int n: next[idx]) {
        if(high[n] == high[idx]-1) {
            temp.push_back(idStr[n]);
            backtracking(idStr, ans, temp, high, next, n, beginIdx);
            temp.pop_back();
        }
    }
}
...
vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
    ...
    if (!getID.count(endWord))
        return ans;


    vector<vector<int>> next(id);
    vector<int> high(id, -1);

//    连接
    for (int i = 0; i < id; i++)
        for (int j = i + 1; j < id; j++) {
            if (Near(idStr[i], idStr[j])) {
                next[i].push_back(j);
                next[j].push_back(i);
            }
        }
...
//    bfs
    queue<int> que;
    que.push(getID[beginWord]);
    high[getID[beginWord]] = 0;
...
//    回溯
    ...
    backtracking(idStr, ans, temp, high, next, getID[endWord], getID[beginWord]);
...
//    bfs
    ...
    vector<int> tmpBegin = {getID[beginWord]};
    que.push(tmpBegin);
    high[getID[beginWord]] = 0;
    ...
    while (!que.empty()) {
        ...
        int last = now[now.size() - 1];

        if (last == getID[endWord]) {
            vector<string> temp;
            for (int i: now)
                temp.push_back(idStr[i]);
            ans.push_back(temp);
        } else {
            for (auto &n: next[last]) {
                if (high[n] >= high[last]+1) {
                    high[n] = high[last] + 1;
                    vector<int> newTemp = now;
                    newTemp.push_back(n);
                    que.push(newTemp);
                }
            }
...
vector<vector<string>> findLadders(string beginWord, string endWord, vector<string> & wordList) {
    ...
    dict.erase(endWord);
    set<string> q1{beginWord}, q2{endWord};
...
//        如果q2中含有此字符串
                    if (q2.count(temp)) {
                        reversed ? next[temp].push_back(s) : next[s].push_back(temp);
                        found = true;
                    }
                    if (dict.count(s)) {
                        reversed ? next[temp].push_back(s) : next[s].push_back(temp);
...
int main() {
    ...
    findLadders("nanny", "aloud", wordList);
